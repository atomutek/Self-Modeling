...
from tensorflow_src.test_planners.test_plan_walker_active import RealerAntWrapper
...
class LearnedAntWrapperRew(gym.Env):
    def __init__(self, ant_env, env_learner=None, loop='real'):
        self.env = ant_env
    ...
    def reset(self):
        self.obs_in = self.env.reset()
        if self.env_learner is not None:
            self.env_learner.reset(self.obs_in)
        self.frame = 0
        self.new_obs = self.obs_in
        return self.obs_in
    ...
    def step(self, action):
        if self.env_learner is not None:
            ...
            self.obs_in, _, _, _ = self.env.step(action)
            if self.loop == 'open2':
                self.new_obs = pred_obs
            elif self.loop == 'closed':
                self.new_obs = self.obs_in
            obs = pred_obs
            r = (self.obs_in[0]/0.3)/60
        else:
            obs, r, done, info = self.env.step(action)
            # old_obs = self.obs_in.copy()
            # self.obs_in = obs.copy()
            # obs = old_obs
            r = (obs[0]/0.3)/60
        self.frame += 1
...
def play(args, env, is_sim=False, pi=None):
    if pi is None:
        pi = train(env_in=env, num_timesteps=1, seed=args.seed, model_path=args.model_path, load=args.load,
...
def main():
    ...
    parser.add_argument('--load', default='models/ant_policy-2019-02-04-13:53:40')
    parser.add_argument('--self', default='models/2019-02-02-18:29:18.ckpt')
    parser.add_argument('--seed', default=0)
    parser.add_argument('--play', default=10)
    parser.add_argument('--num_timesteps', default=int(1e7))
    parser.add_argument('--visualize', default=False)
    parser.add_argument('--chart', default=True)
    ...
    if args.play < 1:
        ...
        if args.self is None:
            ...
            with tf.Session(config=tf_config) as sess:
                saver = tf.train.Saver()
                saver.restore(sess, args.self)
                print('Model: ' + args.self + ' Restored')
                env_learner.initialize(sess, load=True)

                env = LearnedAntWrapperRew(env, env_learner, args.loop)
                env = RewScale(env, 100)
                # print('Running for '+str(num_timesteps)+' steps')
                pi = pposgd_simple.learn(env, policy_fn,
                        max_timesteps=1,
