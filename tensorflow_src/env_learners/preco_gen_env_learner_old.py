...
def predictor(x, h, latent_size):
    # multiply = tf.shape(x[0])[0]
    # initial_h = tf.ones(multiply,1)*h
    rnn_cell = tf.contrib.rnn.GRUCell(latent_size, name='predictor')
    outputs, states = tf.nn.static_rnn(rnn_cell, x, initial_state=h, dtype=tf.float32)
    return outputs, states

def corrector(x, latent_size, h=None):
    rnn_cell = tf.contrib.rnn.GRUCell(latent_size, name='corrector')
    outputs, states = tf.nn.static_rnn(rnn_cell, x, initial_state=h, dtype=tf.float32)
    return outputs, states
...
def decode(x, out_dim, num_components, drop_rate=0.5):
    ...
    # x = tf.layers.dense(x, 1024, name='mlp')
    # x = tf.layers.dropout(x, rate=drop_rate, name='do')
    # x = tf.nn.relu(x, name='rl')
    #
    # x = tf.layers.batch_normalization(x, name='bn4')
    # x = tf.layers.dense(x, 128, name='mlp4')
    ...
    out = tf.layers.dense(out, 128, name='mlp0_'+str(i))
    out = tf.layers.dropout(out, rate=drop_rate, name='do0_'+str(i))
    out = tf.nn.relu(out, name='rl0_'+str(i))

    # out = tf.layers.batch_normalization(out, name='bn0c_'+str(i))
    # out = tf.layers.dense(out, 128, name='mlp0c_'+str(i))
    ...
    out = tf.layers.dense(out, 64, name='mlp1_'+str(i))
    ...
    # out = tf.layers.dense(out, 64, name='mlp1c_'+str(i))
    # out = tf.layers.dropout(out, rate=drop_rate, name='do1c_'+str(i))
    # out = tf.nn.relu(out, name='rl1c_'+str(i))

    # out = tf.layers.batch_normalization(out, name='bn1d_'+str(i))
    # out = tf.layers.dense(out, 32, name='mlp1d_'+str(i))
    # out = tf.layers.dropout(out, rate=drop_rate, name='do1d_'+str(i))
    # out = tf.nn.relu(out, name='rl1d_'+str(i))

    outputs = []
    for i in range(num_components):

        out = tf.layers.batch_normalization(out, name='bn2_'+str(i))
        out = tf.layers.dense(out, 32, name='mlp2_'+str(i))
...
class PreCoGenEnvLearner(EnvLearner):
    def __init__(self, env_in):
        ...
        with tf.variable_scope('PreCoGen', reuse=tf.AUTO_REUSE):
            ...
            corr_out, self.corr_hidden = corrector([self.x], self.latent_size)
            ...
            pred_out, self.pred_hidden = predictor([tmp_a_seq[0]], self.corr_hidden, self.latent_size)
            ...
            for i in range(1, self.max_seq_len):
                a = tmp_a_seq[i]
                y = tmp_y_seq[i]
                pred_out, last_state = predictor([a], last_state, self.latent_size)
            ...
            # Testing
            _, self.corr_hidden_out = corrector([self.x], self.latent_size)
            self.autoencoded = decode(self.corr_hidden_out, self.state_dim, self.num_components)

            _, self.pred_hidden_out = predictor([self.a], self.corr_hidden_out, self.latent_size)
            self.decodeds = decode(self.pred_hidden_out, self.state_dim, self.num_components)

            _, self.pred_hidden_open = predictor([self.a], self.state_in, self.latent_size)
    ...
    def train_epoch(self, data):
        ...
        # start = time.time()
        # for i in range(min(len(X), len(A), len(S))):
        #     single, seq, corr, corr_out, pred_out, single_grads, seq_grads, corr_grads = self.sess.run([self.loss_single, self.loss_seq, self.loss_corr, self.decoded_corrs,self.decoded_preds,
        #                                                 self.single_grads,
        #                                                 self.seq_grads,
        #                                                 self.corr_grads],
        #                              feed_dict={
        ...
        #                                         self.y_seq: S[i]
        #                                        })
        #     single_grad.append([(single_grads[k][0], self.single_grads[k][1]) for k in range(len(single_grads))])
        #     seq_grad.append([(seq_grads[k][0], self.seq_grads[k][1]) for k in range(len(seq_grads))])
        #     corr_grad.append([(corr_grads[k][0], self.corr_grads[k][1]) for k in range(len(corr_grads))])
        #
        #     Single += np.mean(single)
        #     Seq += np.mean(seq)
        #     Corr += np.mean(corr)
        ...
        #     self.sess.run([self.optimizer.apply_gradients(single_grad[i]),
        #                    self.optimizer.apply_gradients(seq_grad[i]),
        #                    self.optimizer.apply_gradients(corr_grad[i]),
        #                    ])
        # return Single / len(X), Seq / len(X), Corr / len(X)
